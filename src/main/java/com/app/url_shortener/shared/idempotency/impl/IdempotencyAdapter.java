...
public class IdempotencyAdapter implements IdempotencyPort {
  ...
  public boolean saveInProgress(IdempotencyKey idempotencyKey, Duration ttl) {
    var entry =
        new IdempotencyEntry(
            IdempotencyStatus.IN_PROGRESS, idempotencyKey.fingerprint(), null, Instant.now());

    return Boolean.TRUE.equals(
        redisTemplate.opsForValue().setIfAbsent(redisKey(idempotencyKey), serialize(entry), ttl));
  }
  ...
  public void saveCompleted(IdempotencyKey idempotencyKey, CachedResponse response, Duration ttl) {
    var entry =
        new IdempotencyEntry(
            IdempotencyStatus.COMPLETED, idempotencyKey.fingerprint(), response, Instant.now());
  ...
  private String serialize(IdempotencyEntry entry) {
    try {
      ...
      throw new IdempotencyCacheException(
          "Erro ao serializar o cache de idempotência para o Redis");
    }
  }
