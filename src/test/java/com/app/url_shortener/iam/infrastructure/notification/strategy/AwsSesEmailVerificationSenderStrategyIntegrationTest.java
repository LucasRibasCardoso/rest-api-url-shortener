...
class AwsSesEmailVerificationSenderStrategyIntegrationTest extends AbstractIntegrationTest {
  ...
  void shouldSendEmailAndExposeCapturedMessageInLocalStack() throws Exception {
    ...
    assertThat(capturedMessage.get("Destination").get("ToAddresses").get(0).asString()).isEqualTo(TO_EMAIL);
    assertThat(capturedMessage.get("Subject").asString()).isEqualTo("Confirme seu e-mail");
    assertThat(capturedMessage.get("Body").get("text_part").asString()).contains(CODE, "10 minutos");
    assertThat(capturedMessage.get("Body").get("html_part").asString()).contains("<strong>" + CODE + "</strong>", "10 minutos");
  }
  ...
  private JsonNode findCapturedMessage(String providerMessageId) throws Exception {
    ...
    var request =
        HttpRequest.newBuilder(
                endpoint.resolve("/_aws/ses?email=" + encodedFromEmail))
            .GET()
            .build();
