...
class UrlErrorCodeTest {
  ...
  class MessageTests {
    ...
    void shouldExposeStandardPortugueseMessages() {
      // 1. Arrange
      Map<UrlErrorCode, String> messages =
          Map.ofEntries(
              ...
              Map.entry(
                  UrlErrorCode.URL_SHORT_CODE_COLLISION, "O código curto informado já existe."),
              ...
              Map.entry(
                  UrlErrorCode.COUNTER_ID_INVALID_RESPONSE,
                  "Resposta inválida do DynamoDB ao alocar bloco de IDs."),
              Map.entry(
                  UrlErrorCode.COUNTER_ID_CONDITIONAL_CHECK_FAILED,
                  "Item do contador de IDs não encontrado no DynamoDB."),
              Map.entry(
                  UrlErrorCode.COUNTER_ID_THROUGHPUT_EXCEEDED,
                  "Capacidade do DynamoDB excedida ao alocar bloco de IDs."),
              Map.entry(
                  UrlErrorCode.COUNTER_ID_TABLE_NOT_FOUND,
                  "Tabela do contador de IDs não encontrada no DynamoDB."),
              Map.entry(
                  UrlErrorCode.COUNTER_ID_DYNAMODB_FAILURE,
                  "Falha do DynamoDB ao alocar bloco de IDs."),
              Map.entry(
                  UrlErrorCode.COUNTER_ID_CLIENT_FAILURE,
                  "Falha de comunicação com o DynamoDB ao alocar bloco de IDs."));
